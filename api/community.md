...
- 当前暂未实现话题后台合并/删除、用户主页隐私设置、商家广告帖权限细分和完整积分流水展示。
...
种草帖 `type=grass` 必须关联至少一个 `product_ids`，且这些商品必须来自发帖用户已完成订单。普通帖和商家动态可以绑定商品，但不产生种草奖励。前端页面应通过商品选择器生成 `product_ids`，不要要求用户手动输入商品 ID；后续可继续优化为已购商品筛选和商品卡片式选择。
